   ...
   <form method="get" action="">
      <div class="container">
         ...
         <div class="row">
            <div class="col-md-1 section-heading text-center to-animate form-inline">
               ...
               <select style="color:#eece49" class="form-control" id="search4" name="favoHo">
                  <option value="" style="display:none">즐겨찾는 병원</option>
         ...
         <br>
          ...
          %>
            <div class="row">
            <% 
               for(int j=0; j<((i==(rowNum-1))?(rList.size() - i*3):3); j++){ 
            	  
                  if(rList.get(j + 3*i).getGood()!=0){
                     goodIcon = "red";
                  } else {
                     goodIcon = "#999";
                  }
                  
                  if(rList.get(j + 3*i).getBad()!=0){
                     badIcon = "blue";
                  } else {
                     badIcon = "#999";
                  }
            ...
            %>
               <input type="hidden" name="reNum" value=<%=rList.get(j + 3*i).getReNum() %>>
               <div class="col-md-4">
                  <div class="box-testimony">
                     <blockquote class="form-inline to-animate-2">
                        <p>&ldquo; <%= rList.get(j + 3*i).getReview() %>&rdquo;</p>
                           ...
                           <span style="float:right"><i class="icon-thumbs-up" onclick="iconChange(this)" name="icon1" title="" style="font-size: 17pt; color:<%=goodIcon %>; cursor:pointer;"></i>
                           &nbsp;<font name="good" size=2; style="color:white;"><%= rList.get(j + 3*i).getgPoint()%></font>&nbsp;&nbsp;
                           <i class="icon-thumbs-down" onclick="iconChange(this)" title="" name="icon2" style="font-size: 17pt; color:<%=badIcon %>; cursor:pointer;"></i>
                           &nbsp;<font name="bad" size=2; style="color:white;"><%= rList.get(j + 3*i).getnPoint()%></font>
                           </span>
                        <br>
                     </blockquote>
                     <div class="author to-animate">
                        ...
                        <p>
                           <%=rList.get(j + 3*i).getUserName()%> &nbsp;<a href="#" title=<%=rList.get(j + 3*i).getHpName() %> name="hospital" onclick="reservHospital(this);"><%=rList.get(j + 3*i).getHpName() %>: 예약하기</a> <span class="subtext" style="font-size: 10pt;"><%=rList.get(j + 3*i).getReDate() %></span>
                        </p>
                     </div>
                  </div>
               </div>
      ...
      <div class="form-group to-animate" align="center">
         ...
         <script>
            ...
            function search(){
                    if($('select[name=favoHo]').val()!=""){
                        keyword = $('select[name=favoHo]').val();
                        location.href="<%=request.getContextPath()%>/rSearch.su?index=&area=&keyword="+keyword;
